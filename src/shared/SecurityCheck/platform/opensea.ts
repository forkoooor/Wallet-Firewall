...
export async function fetchOrders(contract: string, tokenId: string) {
  const req = await fetch("https://opensea.io/__api/graphql/", {
    headers: {
      ...
      "x-signed-query":
        "dca533c8543ae52c1fe8fa97823f7bf281bac34ecd412f5e5a5224070cbfa5c3",
    },
...
async function fetchOffer(contract: string, tokenId: string) {
  ...
  const orders = offer.data.orders.edges
    .map((_: any) => _.node)
    .filter((_: any) => _.side === "BID")
    .map((_: any) => {
      return {
...
export async function checkPage() {
  ...
  const isOpensea = window.location.hostname.includes('opensea.io')
  if (isOpensea && url.indexOf("assets") > -1) {
    const pair = url.split("assets/")[1];
    const [chain, contract, tokenId] = pair.split("/");
    if (chain != 'ethereum') return;
    let orders = await fetchOffer(contract, tokenId);
    // console.log('orders', orders)
    // orders = [
    //   {
    //     makerAddress: "0x4ea3cc382a9c88c33505fcd8c9d4ff16625e9f53",
    //     token: "WETH",
    //     chain: {
    //       identifier: "ETHEREUM",
    //     },
    ...
    if (validOrders.length) {
      return {
        type: "spoofing-offer",
        title: "Spoofing Offer",
        message: `Spoofing offer detected, the maker don't have enough WETH, price: ${firstOrder.priceType.unit} ${firstOrder.token} maker: ${firstOrder.makerAddress}`,
      };
  }
  }
}
