...
function getPageEnv() {
  ...
  let scripts: string[] = [];
  try {
    scripts = Array.from(document.querySelectorAll("script")).map((_) => _.src);
  } catch(e) {}
  return {
    host: window.location.host,
    url: window.location.href,
    /* @ts-ignore */
    chainId: typeof window.ethereum != 'undefined' && window.ethereum.chainId,
    scripts,
...
export default function AlertDialog({ firewall }: { firewall: Inspector }) {
  ...
  useEffect(
    () => {
      let check = async () => {
        const result = await checkPage(true);
        if (result.length) {
  ...
  useEffect(() => {
    ...
    const doCheck = async () => {
      ...
      const allResults = checkResults.filter(_ => _);
      if (hasIssues.length) {
    ...
    <ThemeProvider theme={darkModeTheme}>
      ...
      >
        ...
        <DialogActions>
          ...
          >
            <Button
              onClick={() => setPageCheckResult(null)}
              variant="outlined"
